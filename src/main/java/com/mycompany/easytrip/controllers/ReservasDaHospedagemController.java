...
public class ReservasDaHospedagemController {
    private final TelaReservasDaHospedagem tela;
    
    public ReservasDaHospedagemController(TelaReservasDaHospedagem tela){
        this.tela = tela;
    }
    
    public void carregarReservas(int hospedagemId){
        try(EntityManager entityManager = MinhaConexao.getEntityManager()){
            ReservaServico servico = new ReservaServico(entityManager);
            
            List<Reserva> reservas = servico.listarReservasDaHospedagem(hospedagemId);
